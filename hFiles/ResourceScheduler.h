...
public:
    ...
    vector< vector<double> > oneHostJobCostTable; // Job time cost table when using different number of cores .
    // Job -> core numbers(on one host) -> time cost
    vector< vector<double> > oneHostLocalOpt;

	ResourceScheduler(int,int);
    void oneHostScheduler();
    void oneHostSchedulerGreedy1();
	void schedule();
...
//	void visualization(float max); // An optional fuction.
    ...
    void getOneHostJobCostTable(); // output Scheduler Table on one Host
